...
This UI plan targets the backend's current fanotify release after rewrite phases 1 and 2. It deliberately excludes the future LSM phase: the dashboard must present only File Access and File Execute, not Read or Write.

## Current backend contract

- The backend emits only File Access (`FAN_OPEN_PERM`) and File Execute (`FAN_OPEN_EXEC_PERM`) decisions. Its rewrite removes `FAN_ACCESS_PERM`, the `InterceptReads` setting, and runtime `OpWrite` events.
- `echo test > somefile` is a File Access decision. It may open for writing, but fanotify does not reveal that mode, so the same Access rule covers opens for reading, writing, or both.
- Read and Write are not dashboard metrics in this release. Write is hidden from the user-facing rule model until future LSM enforcement makes it real.
...
- The required backend Phase 2.5 persists mount ID and mount path with every Access and Execute record. Recent Mount Protection and Protected Mounts depend on that phase and must not ship before it does.
...
| `charts` / Connections Tunneled through SPN | **Accesses and Executes over Time** | The current panel is hidden unless SPN is enabled. Replace it with an always-rendered File Access and File Execute chart in the same second panel position. |
...
| `countries` / Recent Connections per Country | **Recently Protected Mounts** | do what portmaster does, but "protections" instead of connections, and mounts instead of country |
...
| `bwvis-line` / Recent Bandwidth Usage | **File Accesses and Executes** | Keep the large slot for an operation-focused time chart: File Access and File Execute only. |
...
| Mount path | Coverage | Access allowed | Access blocked | Execute allowed | Execute blocked |
| --- | --- | --- | --- | --- | --- |
...
coverage badge warning/red even if its recent activity counts are zero. The
compact **Recent Mount Protection** card links into this detailed view.
...
| Data Received | **Access Allowed** | Allowed File Access decisions. |
| Data Sent | **future write allowed** | blank or todo text for now. |
...
│ Access + Execute active     │ │ 0 overload denies          │ │ response latency: 4 ms      │
└─────────────────────────────┘ └────────────────────────────┘ └────────────────────────────┘
```
...
1. **Enforcement State** — lifecycle/degraded state and warnings. It identifies the active current-release enforcement as Access and Execute. Mount coverage is intentionally shown only in **Protected Mounts**, not duplicated here.
...
7. Implement this UI after the fanotify backend rewrite and before any LSM work; expose only current File Access and File Execute semantics.
...
2. **Write activity:** defer this entirely to the backend plan's future LSM phase. The current UI must not add a fanotify write metric or show a Write card before that enforcement exists.
