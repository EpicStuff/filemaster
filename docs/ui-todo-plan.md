...
| `charts` / Connections Tunneled through SPN | **Opens and Executes over Time** | The current panel is hidden unless SPN is enabled. Replace it with an always-rendered Open and Execute chart in the same second panel position. |
...
| `bwvis-line` / Recent Bandwidth Usage | **Opens and Executes** | Keep the large slot for an operation-focused time chart: Open and Execute only. |
...
7. Implement this UI after the fanotify backend rewrite; expose only current
