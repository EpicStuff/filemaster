...
9. Make no-match handling structurally use `DecisionSnapshot.DefaultAction` for every operation and every snapshot construction path, including directly constructed snapshots; do not rely on copied or zero-value per-list defaults. Test unmatched Read/Write/Execute under Permit, Block, and Ask.
