...
func TestSaveLoadRoundTrip(t *testing.T) {
	...
	if got := h1.RulesFor("/usr/bin/vim"); len(got) != 1 || got[0].Pattern != "/home/alice/notes\\?.txt" {
		t.Fatalf("vim literal rule: %+v", got)
	}
	...
	p2.responses = map[string]string{"/home/alice/notes?.txt": ActionDeny, "/home/alice/notesA.txt": ActionDeny}
	...
	if v := h2.Decide(context.Background(), &FileEvent{Exe: "/usr/bin/vim", Path: "/home/alice/notesA.txt"}); v != VerdictDeny {
		t.Errorf("similar path after reload: %s, want deny from prompt", v)
	}
	if p2.called != 2 {
		t.Errorf("prompter called %d times, want 2 (literal rule must not match glob lookalike)", p2.called)
	}
}
...
func TestLoadWrongVersionRejected(t *testing.T) {
	...
	if err := os.WriteFile(rulesPath, []byte(`{"version": 2, "rules": {}}`), 0o600); err != nil {
		t.Fatal(err)
	}
