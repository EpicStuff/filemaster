// Ported from service/netquery/query_request.go.
// Changes: package name, orm import path, generateSQL uses schema.Name instead of
// hardcoded "connections", databases always resolve to "main" (single DB).
package filequery
...
type (
	// QueryRequestPayload describes the payload of a filequery query.
	QueryRequestPayload struct {
		...
		// Databases is accepted for API compatibility but always resolves to LiveDatabase.
	...
	// BatchQueryRequestPayload describes the payload of a batch filequery query.
...
func (req *QueryRequestPayload) generateSQL(ctx context.Context, schema *orm.TableSchema) (string, map[string]interface{}, error) {
	...
	// File-access events use a single "main" database regardless of what was requested.
	source := fmt.Sprintf("SELECT * FROM main.%s %s", schema.Name, whereClause)
