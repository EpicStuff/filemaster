...
func (p *Process) IsPortmasterUi(ctx context.Context) bool {
	...
	for i := 0; i < checkLevels; i++ {
		...
		if err == nil && realPath == module.portmasterUIPath {
			...
			log.Tracer(ctx).Warningf("process: %d %q is a child of the Filemaster UI, but does not have the PORTMASTER_UI_WEBVIEW_PROCESS environment variable set. Ignoring.", p.Pid, p.Path)
			return false
		}
